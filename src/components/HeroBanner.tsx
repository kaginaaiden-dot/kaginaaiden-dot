      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-8 pt-12 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
          <div className="lg:col-span-7 space-y-4">
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black tracking-tight text-white leading-tight font-serif">
              Elevate Your Way of <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 via-amber-200 to-[#E88B23]">Discovering The World</span>
            </h1>
            <p className="text-emerald-100/90 text-sm sm:text-base max-w-xl leading-relaxed">
              From private Serengeti safaris to Kyoto luxury ryokans and Amalfi yacht escapes. Experience seamless booking, AI-tailored itineraries, and 24/7 dedicated concierge care.
            </p>
          </div>
          ...
          <div className="lg:col-span-5 flex flex-wrap gap-6 items-center lg:justify-end text-xs text-emerald-200 border-t lg:border-t-0 lg:border-l border-emerald-800/60 pt-4 lg:pt-0 lg:pl-6">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="font-bold text-white text-sm">15,000+</div>
                <div className="text-[11px] text-emerald-300/80">Tailored Journeys</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-5 h-5 text-emerald-400" />
              <div>
                <div className="font-bold text-white text-sm">99.4%</div>
                <div className="text-[11px] text-emerald-300/80">Satisfaction Rate</div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <HeartHandshake className="w-5 h-5 text-amber-300" />
              <div>
                <div className="font-bold text-white text-sm">24/7 VIP</div>
                <div className="text-[11px] text-emerald-300/80">Global Support</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-10 bg-white rounded-2xl p-4 sm:p-6 text-gray-900 shadow-2xl border border-emerald-100 max-w-5xl">
          ...
          <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-center">
            ...
            <div className="lg:col-span-5 bg-gray-50 p-3 rounded-xl border border-gray-200 hover:border-emerald-500 transition-colors">
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider block mb-1 flex items-center gap-1">
                <MapPin className="w-3 h-3 text-[#0D3B2B]" /> Where to next?
              </label>
              <input
                type="text"
                placeholder="e.g. Kyoto, Serengeti, Positano, Bali..."
                value={destination}
            ...
            <div className="lg:col-span-4 bg-gray-50 p-3 rounded-xl border border-gray-200 hover:border-emerald-500 transition-colors">
              <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider block mb-1 flex items-center gap-1">
                <Compass className="w-3 h-3 text-[#E88B23]" /> Travel Style
              </label>
              <select
                value={category}
              ...
              >
                ...
                <option value="Culture">Cultural Heritage & Tea</option>
                <option value="Wellness">Wellness & Spa Resorts</option>
                <option value="Mountain">Mountain & Ski Escapes</option>
              </select>
            </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-gray-500 pt-2 border-t border-gray-100">
            ...
            {['Serengeti Safari', 'Kyoto Tea Tour', 'Amalfi Private Yacht', 'Swiss Glacier Express', 'Bali Eco Villa'].map((tag) => (
              <button
                key={tag}
                onClick={() => {
